...
//         vrai( vAttr.isPrivate(), "$a $n n'est pas prive !" );
    ...
    public static void verifNomAttribut( final FieldContent pF )
    {
        ...
        sNom = pF.fieldName();
        boolean vOK = sNom.startsWith("a") && sNom.length()>1
...
//         failIfNot();
    ...
    @Test
    public static Object getAndVerifIns1io( final ConstructorContent pC, final FieldContent pF,
                                            final Object pVin, final Object pVout )
    {
        Object vIns = null;
        try {
            vIns = pC.newInstance( new Object[]{ pVin } );
        } catch( final Exception pE ) {
            fail( "le constructeur " + pC.getName() + pC.getParameterString() + " genere une exception !" );
        } // t/c
        vrai( pVout.toString().equals( pF.fieldValue( vIns ) ), "Le constructeur n'initialise pas correctement l'attribut" + pF );
        failIfNot();
        return vIns;
    } // getAndVerifIns1io(....)
...
//         vrai( verifModMethod( vMe, "public", "static final" ), "*" );
    ...
    public static void verifOverrideDebug( String pFil, String pType, String pMet, boolean pDebug )
    {
        ...
        Scanner vJ = null;
        try {
          vJ = new Scanner( new File( pFil ), "UTF-8" );
...
// ===== entrees/sorties =====zfil
    ...
    public String compareTerminal( String refName )
    {
        try {
            ...
            Scanner ref = new Scanner( new File( refName ), "UTF-8" );
            Scanner res = new Scanner( new File( "test.out" ), "UTF-8" );
    ...
    public static String removeComment( final String pS )
    {
        int vPos = pS.indexOf( "//" );
        if ( vPos == -1 )  return pS;
        return pS.substring( 0, vPos );
    } // removeComment(.)
...
//        System.out.println(System.getProperty("user.dir"));
    ...
    public static void verifExtendsDebug( String pFil, String pCla, boolean pDebug ) // un seul parametre
    {
        ...
        final String vAvec = ".*"+"\\s+class\\s+"+sClasse+"\\s+extends\\s+"+pCla+"\\b.*";
        final String vSans = ".*"+"\\s+class\\s+"+sClasse                       +".*";
...
//        final String vSans = ".*"   +"\\s+"+pMet+"\\(.*";
        ...
        try {
          ...
          if ( vCpt == 0 )
            error( "code source de "+pFil+" non analysable : " + "fichier vide ??" );
          else if ( vCount < pN ) {
            sRetour = null;
